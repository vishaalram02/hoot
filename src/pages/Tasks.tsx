...
export function Tasks({ claimTasks, data, selection, setSelection, setDirData}: TaskProps){
    ...
    const preview = () => {
        if(!popUp) return <></> ;
        try{
            return <Preview setDirData = {setDirData} claimTasks = {claimTasks} data = {data.filter((item) => (selection.filter((id) => (item.id === id)).length > 0) || item.claimedby === name)} setSelection = {setSelection} setPopUp = {setPopUp}></Preview>
        } catch{
            return <></>;
        }
    };
